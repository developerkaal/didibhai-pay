...
import { getQuote, createTransaction } from "@/lib/api";
...
export default function SendMoneyPage() {
  ...
  useEffect(() => {
    const fetchQuote = async () => {
      ...
      setLoading(true);
      try {
        const res = await getQuote(amount, isIndiaToNepal ? "IN-NP" : "NP-IN");
        setQuote({
          rate: res.data.rate,
          fee: res.data.feeTotal,
  ...
  }, [amount, direction, isIndiaToNepal, toast]);

  const rate = quote.rate;
  const fee = quote.fee;
  const receiveAmount = quote.payoutAmount;
  ...
  const handlePayment = async () => {
    try {
      const res = await createTransaction({
        ...
        direction: isIndiaToNepal ? "IN-NP" : "NP-IN",
        senderDetails: {
          id: user?.id,
          name: user?.fullName,
          email: user?.email,
      ...
      // Ideally redirect to status page
      // window.location.href = `/track?id=${res.data.id}`;
    } catch (err) {
            ...
            {steps.map((step, index) => (
              <div key={step.key} className="flex items-center">
                <div className="flex flex-col items-center">
                  ...
                  <span
                    className={`text-xs mt-2 ${index <= currentStepIndex ? "text-foreground font-medium" : "text-muted-foreground"}`}
                  >
                    {step.label}
                  </span>
                </div>
                {index < steps.length - 1 && (
                  <div
                    className={`w-full h-0.5 mx-2 ${index < currentStepIndex ? "bg-accent" : "bg-border"}`}
                    style={{ minWidth: "40px" }}
                  />
                )}
              </div>
        ...
        <div className="bg-card rounded-2xl border border-border shadow-lg p-6 lg:p-8">
                  ...
                  <div className="p-4 bg-secondary/50 rounded-xl space-y-2 text-sm">
                    ...
                    <div className="flex justify-between pt-2 border-t border-border font-semibold">
                      ...
                      <span className="text-accent">
                        {quote.totalToPay.toFixed(2)} {sendCurrency}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">
              ...
              <div className="space-y-6">
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      placeholder={
                        isIndiaToNepal ? "+977 98XXXXXXXX" : "+91 98XXXXXXXX"
                      }
                      className="mt-1"
                    />
                  </div>
                ...
                <div>
                  <Label className="mb-3 block">
                    How should recipient receive money?
                  </Label>
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    >
                      <Building2
                        className={`w-6 h-6 ${recipientDetails.payoutMethod === "bank" ? "text-accent" : "text-muted-foreground"}`}
                      />
                    ...
                    >
                      <CreditCard
                        className={`w-6 h-6 ${recipientDetails.payoutMethod === "upi" ? "text-accent" : "text-muted-foreground"}`}
                      />
                      <span className="text-sm font-medium">
                        {isIndiaToNepal ? "eSewa/Khalti" : "UPI"}
                      </span>
                    </button>
                    <button
                      onClick={() =>
                    ...
                    >
                      <Wallet
                        className={`w-6 h-6 ${recipientDetails.payoutMethod === "wallet" ? "text-accent" : "text-muted-foreground"}`}
                      />
                  ...
                  <div className="grid md:grid-cols-2 gap-4 p-4 bg-secondary/50 rounded-xl">
                    ...
                    <div>
                      <Label htmlFor="ifscCode">
                        {isIndiaToNepal ? "Bank Code" : "IFSC Code"}
                      </Label>
                      <Input
                        id="ifscCode"
                ...
                {recipientDetails.payoutMethod === "upi" && (
                  <div className="p-4 bg-secondary/50 rounded-xl">
                    <Label htmlFor="upiId">
                      {isIndiaToNepal ? "eSewa/Khalti ID" : "UPI ID"}
                    </Label>
                    <Input
                      id="upiId"
          ...
          {currentStep === "review" && (
            <div className="animate-fade-in">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-start gap-3 p-4 bg-accent/5 rounded-xl border border-accent/20">
                  ...
                  <p className="text-sm text-muted-foreground">
                    ...
                    Policy. Transfer is subject to verification and may take up
                    to 24 hours.
                  </p>
                </div>
              </div>
              ...
              >
                Confirm & Pay {quote.totalToPay.toFixed(2)} {sendCurrency}
              </Button>
            </div>
          )}
        </div>
